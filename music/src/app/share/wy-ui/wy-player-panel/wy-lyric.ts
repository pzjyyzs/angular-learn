...
import { from, skip, Subject, Subscription, timer, zip } from "rxjs";
import { Lyric } from "src/app/services/data-types";
...
export class WyLyric {
  ...
  private generLyric() {
    ...
    lines.forEach((line: string) => this.makeLine(line));
  }

  private generTLyric() {
    const lines = this.lrc.lyric.split('\n');
    const tlines = this.lrc.tlyric.split('\n').filter((item: string) => this.timeExp.exec(item) !== null);
    ...
    const first = this.timeExp.exec(tempArr[1][0])![0];

    const skipIndex = tempArr[0].findIndex(item => {
      const exec = this.timeExp.exec(item);
      if (exec) {
        return exec[0] === first;
      }
      return false;
  ...
  private makeLine(line: string, tline = '') {
    const result = this.timeExp.exec(line);
    if (result) {
      const txt = line.replace(this.timeExp, '').trim();
      const txtCn = tline ? tline.replace(this.timeExp, '').trim() : '';
      if (txt) {
